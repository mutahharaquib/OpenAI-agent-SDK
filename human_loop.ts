...
const sendEmailTool = tool({
  ...
  execute: async ({ to, subject, html }) => {
    const API_KEY = process.env.AUTOSEND_API_KEY; // API key from https://autosend.com/
    const response = await axios.post(
      'https://api.autosend.com/v1/mails/send',
      {
        from: {
          email: 'mohammadmutahharaquib@gmail.com',
...
async function main(q: string) {
  ...
  while (hasInteruptions) {
    ...
    for (const interput of result.interruptions) {
      if (interput.type === 'tool_approval_item' && interput.rawItem.type === 'function_call') {
...
main('What is the weather in Hyderabad and send it to me on mutahharaquib@gmail.com');